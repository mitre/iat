...
class irisRecognition(object):
    ...
    def bioMechCartToPol_torch(self, image, mask, pupil_xyr, iris_xyr, radiusBiomech, interpolation='bilinear'):       
        with torch.no_grad():
            ...
            theta = (2*pi*torch.linspace(1,polar_width,polar_width)/polar_width).to(self.device)
            ...
            radius = torch.tensor(radiusBiomech).float().reshape(-1, 1).to(self.device)  #64 x 1
            
            pxCoords = torch.matmul((1-radius), pxCirclePoints.reshape(-1, 1, polar_width)).to(self.device) # b x 64 x 512
            pyCoords = torch.matmul((1-radius), pyCirclePoints.reshape(-1, 1, polar_width)).to(self.device) # b x 64 x 512
            
            ixCoords = torch.matmul(radius, ixCirclePoints.reshape(-1, 1, polar_width)).to(self.device) # b x 64 x 512
            iyCoords = torch.matmul(radius, iyCirclePoints.reshape(-1, 1, polar_width)).to(self.device) # b x 64 x 512
            ...
            image_polar = self.grid_sample(image, grid_sample_mat, interp_mode=interpolation).to(self.device)
