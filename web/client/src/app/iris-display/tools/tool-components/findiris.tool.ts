...
@Component({
  template: `
    <ng-container *ngIf="_isActive"></ng-container>
  `
})
export class FindIrisTool extends IrisDrawingTool implements RequiredMembers {

  static icon = 'fa-eye';
  static id = 'FindIris';
  static type: IrisToolType = IrisToolType.ANNOTATION;
  name = 'FindIris'; //this needs to match the name in the review config json
  ...
  onToolInit() {
    this.initMathTables();
    const canvas = this.toolManager._project.view.element;
    const imageData: ImageData = this.toolManager._iris.getImageData(new Rectangle(0, 0, canvas.width, canvas.height));

    const ret = this.findIris(imageData);

    const ctx = this.toolManager._iris.canvas.getContext('2d', {willReadFrequently: true});
    ctx.beginPath();
    ctx.arc(this.IrisCenterX, this.IrisCenterY, this.IrisRadius, 0, 2 * Math.PI);
    ctx.stroke();

    ctx.beginPath();
    ctx.arc(Math.floor(this.RoughPcx), Math.floor(this.RoughPcy), Math.floor(this.RoughPrad), 0, 2 * Math.PI);
    ctx.stroke();
  }
  ...
  findPupilCenter(edgeData: ImageNumberArray, irisCenterX: number, irisCenterY: number, irisDiameter: number, dsImage: ImageArray) {
    ...
    for (rdx = MinPrtrial; rdx < MaxPrtrial; rdx++) {
      ...
      for (idx = roileft; idx < roiright; idx++) {
        for (idy = roitop; idy < roibottom; idy++) {
          ...
          if (trialresponse > bestresponse) {
            console.log('Setting pupil values', pupilCx, pupilCy, pupilRad);
            bestresponse = trialresponse;
    ...
    this.RoughPcx = pupilCx;
    this.RoughPcy = pupilCy;
    this.RoughPrad = pupilRad;
    this.RoughPdiam = pupilRad * 2;
  }
