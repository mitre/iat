...
{    
                                         ...
                                         int maxPupilDiameter )
    {
       ...
       // OsiStringUtils str ;
        ...
        else if ( maxIrisDiameter > (int)(check_size = (int)floor((float)min(pSrc->height,pSrc->width))) )
        {          
			// uncomment "cout" lines if you want to have this warning displayed in command line
        ...
        {
            maxPupilDiameter = (int)(OSI_MAX_RATIO_PUPIL_IRIS * maxIrisDiameter) ;
        }
        
        // Change maxPupilDiameter if it is too big relative to maxIrisDiameter and OSI_MAX_RATIO_PUPIL_IRIS
        else if ( maxPupilDiameter > (check_size = (int)(OSI_MAX_RATIO_PUPIL_IRIS*maxIrisDiameter)) )
        {
			// uncomment "cout" lines if you want to have this warning displayed in command line
        ...
        if ( minPupilDiameter < (int)(check_size = (int)(minIrisDiameter*OSI_MIN_RATIO_PUPIL_IRIS)) )
        {
			// uncomment "cout" lines if you want to have this warning displayed in command line
        ...
        {
			throw runtime_error("Error in function segment : minIrisDiameter = " + 
				std::to_string(minIrisDiameter) + 
				" should be lower than maxIrisDiameter = " + 
				std::to_string(maxIrisDiameter));
        }
        ...
        cvSetImageROI(clone_src,cvRect((int)(rPupil.getCenter().x-3.0/4.0*maxIrisDiameter/2.0),
                                       (int)(rPupil.getCenter().y-3.0/4.0*maxIrisDiameter/2.0),
                                       (int)(3.0/4.0*maxIrisDiameter),
                                       (int)(3.0/4.0*maxIrisDiameter))) ;
        ...
        theta.clear() ;
        theta_step = (float)(360.0 / OSI_PI / rPupil.getRadius()) ;
        ...
        theta.clear() ;
        theta_step = (float)(360.0 / OSI_PI / rPupil.getRadius() * 2) ;
        for ( float t = 0 ; t < 360 ; t += theta_step )
        {
        ...
        theta.clear() ;
        int min_radius = max<int>((int)(rPupil.getRadius()/OSI_MAX_RATIO_PUPIL_IRIS),(int)(minIrisDiameter/2)) ;
        int max_radius = min<int>((int)(rPupil.getRadius()/OSI_MIN_RATIO_PUPIL_IRIS),(int)(3*maxIrisDiameter/4)) ;
        theta_step = (float)(360.0 / OSI_PI / min_radius) ;
        for ( float t = 0 ; t < 360 ; t += theta_step )
        {
        ...
        theta.clear() ;
        theta_step = (float)(360.0 / OSI_PI / rIris.getRadius()) ;
        for ( float t = 0 ; t < 360 ; t += theta_step )
        {
            theta.push_back(t*OSI_PI/180) ;
        }        
                                     ...
                                     const OsiCircle & rIris )
    {
        ...
        for ( int j = 0 ; j < pDst->width ; j++ )
        {
            ...
            {    
                ...
                x = (int) ((1-radius) * point_pupil.x + radius * point_iris.x) ;
                y = (int) ((1-radius) * point_pupil.y + radius * point_iris.y) ;
    ...
    {    
        ...
        for ( int s = -shift ; s <= shift ; s++ )
        {
            ...
            float mean = (float)((cvSum(result).val[0])/(cvSum(mask).val[0])) ;
    ...
    CvPoint OsiProcessings::convertPolarToCartesian ( const CvPoint & rCenter ,
                                                            ...
                                                            float rTheta )
    {
        int x = (int)(rCenter.x + rRadius * cos(rTheta)) ;
        int y = (int)(rCenter.y - rRadius * sin(rTheta)) ;
        return cvPoint(x,y) ;
    }
    ...
    {        
        ...
        if ( maxPupilDiameter == 0 )
        {
            maxPupilDiameter = (int)(min(pSrc->height,pSrc->width) * OSI_MAX_RATIO_PUPIL_IRIS) ;
        }
        ...
        {
            int newmaxPupilDiameter = (int)floor(min(pSrc->height,pSrc->width)*OSI_MAX_RATIO_PUPIL_IRIS) ;
        ...
        {
			throw runtime_error("Error in function detectPupil : minPupilDiameter = " +
                                   std::to_string(minPupilDiameter) +
                                   " should be lower than maxPupilDiameter = " +
									std::to_string(maxPupilDiameter)) ;
        }       
        ...
        IplImage * resized = cvCreateImage(cvSize((int)(pSrc->width*scale),(int)(pSrc->height*scale)),pSrc->depth,1) ;
        cvResize(pSrc,resized) ;

        // Rescale sizes
        maxPupilDiameter = (int)(maxPupilDiameter * scale) ;
        minPupilDiameter = (int)(minPupilDiameter * scale) ;
        ...
        {
            float x = (float)(i - (filter_size-1)/2) ;
            for ( int j = 0 ; j < fh->cols ; j++ )
            {
                float y = (float)(j - (filter_size-1)/2) ;
                if ( x != 0 || y != 0 )
                {
        ...
        // Rescale circle        
        int x = (int)(( (float) ( rPupil.getCenter().x * (pSrc->width-1) ) ) / (filled->width-1) + (float)((1.0/scale)-1)/2) ;
        int y = (int)(( (float) ( rPupil.getCenter().y * (pSrc->height-1) ) ) / (filled->height-1) + (float)((1.0/scale)-1)/2) ;
        int r = (int)(rPupil.getRadius() / scale) ;
    ...
    {
        // Result image
        IplImage * result = cvCreateImage(cvSize((int)(rTheta.size()),maxRadius-minRadius+1),pSrc->depth,1) ;
        cvZero(result) ;
    ...
    void OsiProcessings::runViterbi ( const IplImage * pSrc , vector<int> & rOptimalPath )
    {
        ...
        // Backward process
        for ( int w = (int)(rOptimalPath.size()) - 2 ; w >= 0 ; w-- )
        {
    ...
    void OsiProcessings::drawContour ( IplImage * pImage , const vector<CvPoint> & rContour , const CvScalar & rColor , int thickness )
    {
        ...
        if ( thickness < 0 )
        {
            ...
            cvFillConvexPoly(pImage,points,(int)(rContour.size()),rColor) ;
            delete [] points ;
        }
