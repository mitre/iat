...
namespace osiris
{
    ...
    void OsiCircle::computeCircleFitting ( const vector<CvPoint> & rPoints )
    {
        ...
        float uc = (float) (0.5 * ( suv * ( svvv + suuv ) - svv * ( suuu + suvv ) ) / ( suv * suv - suu * svv ));
		float vc = (float) (0.5 * (suv * (suuu + suvv) - suu * (svvv + suuv)) / (suv * suv - suu * svv));

        // Circle parameters
        setCenter(cvPoint((int)(uc+mx),(int)(vc+my))) ;
